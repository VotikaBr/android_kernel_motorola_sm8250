...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Avoid extra ksu checks once /sdcard/Android is accessible */
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Skip repeated ksu checks if /sdcard/Android is already accessible */
	if (READ_ONCE(susfs_is_sdcard_android_data_decrypted))
		goto skip_checking_for_ksu_proc;

	/* If still locked, tag mounts created by ksu processes */
	if (susfs_is_current_ksu_domain()) {
